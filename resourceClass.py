...
class Resource(object):
    ...
    def draw(self, canvas):
        ...
        if not self.mob:
            canvas.create_text(self.cx, self.cy, text=str(
                self.number), font='Chalkduster 20')
        else:
            canvas.create_text(self.cx, self.cy, text='MOB',
                               font='Chalkduster 20', fill='red')
...
class TradingPort(Seatile):
    ...
    def draw(self, canvas):
        canvas.create_polygon(self.pointList, fill=self.color, outline='black')
        canvas.create_text(self.cx, self.cy, text='3:1 Trade',
                           font='Chalkduster 10')
