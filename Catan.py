...
class Bar(object):
    ...
    def draw(self, canvas, text='', font='Chalkduster 15'):
        canvas.create_rectangle(self.x0,self.y0,self.x1,self.y1, fill=self.fill, width=3)
        canvas.create_text((self.x0+self.x1)/2, (self.y0+self.y1)/2, text=text, font=font)        
    def inBar(self,x,y):
...
def init(data):
    ...
    data.colorList = ['OliveDrab1', 'brown4', 'gold', 'grey', 'chartreuse4']*3 + ['OliveDrab1', 'gold', 'chartreuse4']
    data.playerColorList = [None, 'yellow', 'white', 'blue', 'pink']
    data.menuBar1 = Bar(data.width/3, data.height/3, data.width/3*2, data.height/2)
    data.menuBar2 = Bar(data.width/3, data.height/2, data.width/3*2, data.height/3*2)
    data.helpOnMapSelectionBar = Bar(data.width/10*8-data.width/100, data.height/5, data.width/40*39+data.width/100, data.height/6*5, 'SkyBlue1')
    data.mapConfirm = Bar(data.width/8*3, data.height/7*6, data.width/8*5, data.height/7*6.5, 'SkyBlue1')
    data.returnMenu = Bar(data.width/20, data.height/15, data.width/10*3, data.height/10, 'SkyBlue1')
    data.helpOnMap = Bar(data.width/10*7, data.height/15, data.width/20*19, data.height/10, 'SkyBlue1')
    data.turnBar = Bar(data.width/20, data.height/15, data.width/10*3, data.height/10, 'SkyBlue1')
    data.diceBar = Bar(data.width/10*8, data.height/15, data.width/20*19, data.height/10, 'SkyBlue1')
    data.buildBar = Bar(data.width/20, data.height/7*6, data.width/5, data.height/7*6.5, 'SkyBlue1')
    data.tradeBar = Bar(data.width/10*3, data.height/7*6, data.width/20*9, data.height/7*6.5, 'SkyBlue1')
    data.useBar = Bar(data.width/20*11, data.height/7*6, data.width/10*7, data.height/7*6.5, 'SkyBlue1')
    data.endBar = Bar(data.width/5*4, data.height/7*6, data.width/20*19, data.height/7*6.5, 'SkyBlue1')
    data.resourceBar = Bar(data.width/10*8-data.width/100, data.height/5, data.width/40*39+data.width/100, data.height/6*5, 'SkyBlue1')
    data.playSelectionBar = Bar(data.width/40, data.height/5, data.width/5, data.height/6*5, 'SkyBlue1')
    data.buildRoadSelectionBar = Bar(data.width/40, data.height/5, data.width/5, data.height/120*43, 'SkyBlue1')
    data.buildSettlementSelectionBar = Bar(data.width/40, data.height/120*43, data.width/5, data.height/60*31, 'SkyBlue1')
    data.buildCitySelectionBar = Bar(data.width/40, data.height/60*31, data.width/5, data.height/40*27, 'SkyBlue1')
    data.buildDevelopSelectionBar = Bar(data.width/40, data.height/40*27, data.width/5, data.height/6*5, 'SkyBlue1')
    data.bankTradeBar = Bar(data.width/40, data.height/5, data.width/5, data.height/120*43, 'SkyBlue1')
    data.portTradeBar = Bar(data.width/40, data.height/120*43, data.width/5, data.height/60*31, 'SkyBlue1')
    data.playerTradeBar = Bar(data.width/40, data.height/60*31, data.width/5, data.height/40*27, 'SkyBlue1')
    data.closeTradeBar = Bar(data.width/40, data.height/40*27, data.width/5, data.height/6*5, 'SkyBlue1')
    data.moveMobBar = Bar(data.width/40, data.height/5, data.width/5, data.height/120*43, 'SkyBlue1')
    data.feastBar = Bar(data.width/40, data.height/120*43, data.width/5, data.height/60*31, 'SkyBlue1')
    data.extraVPBar = Bar(data.width/40, data.height/60*31, data.width/5, data.height/40*27, 'SkyBlue1')
    data.closeUseBar = Bar(data.width/40, data.height/40*27, data.width/5, data.height/6*5, 'SkyBlue1')
    data.woolBar = None
    data.brickBar = None
    data.grainBar = None
    data.oreBar = None
...
def timerFired(data):
    while (serverMsg.qsize() > 0):
        ...
        if command == 'Mode':
            ...
            data.otherPlayers[newPID] = playerClass.Player(newPID, data.playerColorList[int(newPID[-1])])
        elif command == 'Ready':
            data.otherReady[msg[1]] = True
        elif command == 'myIDis':
            PID = msg[1]
            data.playerList.append(PID)
            data.me = playerClass.Player(PID, data.playerColorList[int(PID[-1])])
            ...
            if msg[2] == 'kill':
                ...
                if isinstance(data.resourceList[oldMobIndex], resourceClass.Desert):
                    ...
                    data.resourceList[oldMobIndex].mob = False
                    data.resourceList[oldMobIndex].number = data.resourceList[oldMobIndex].oldNumber
            if msg[2] == 'move':
...
def menuRedrawAll(canvas, data):
    ...
    canvas.create_text(data.width/2, data.height/5, text='Settlers of Catan', font='Chalkduster 30 bold', fill='red')
    canvas.create_text(data.width/2, data.height/10*9, text='Proudly Produced by Dunmin Zhu', font='Chalkduster 30 bold', fill='white')
    data.menuBar1.draw(canvas,'Play with Computer', font='Chalkduster 20 bold')
    data.menuBar2.draw(canvas, 'Play with your friends', font='Chalkduster 20 bold')
    # The AI part is one of the feature to be added in the future
    if data.underConstruction:
        canvas.create_text(data.width/2, data.height/7, text='Still Under Construction...', font=('Apple Chancery',20), fill='blue')

def drawResource(canvas, data):
    PID = data.me.PID
    color = data.me.color
    canvas.create_text(data.width/80*71, data.height/4.3, text='Resource List for %s' % (PID), fill=color)
    canvas.create_text(data.width/80*71, data.height/20*7, text='You have %d wool' % data.me.resource['wool'], fill=color)
    canvas.create_text(data.width/80*71, data.height/20*8, text='You have %d brick' % data.me.resource['brick'], fill=color)
    canvas.create_text(data.width/80*71, data.height/20*9, text='You have %d grain' % data.me.resource['grain'], fill=color)
    canvas.create_text(data.width/80*71, data.height/20*10, text='You have %d ore' % data.me.resource['ore'], fill=color)
    canvas.create_text(data.width/80*71, data.height/20*11, text='You have %d lumber' % data.me.resource['lumber'], fill=color)
    canvas.create_text(data.width/80*71, data.height/20*12, text='You have %d city(s)' % data.me.city, fill=color)
    canvas.create_text(data.width/80*71, data.height/20*13, text='You have %d settlement(s)' % data.me.settlement, fill=color)
    canvas.create_text(data.width/80*71, data.height/20*14, text='You have %d VP(s)' % data.me.VP, fill=color)

def drawBuildMenu(canvas, data):
    buildSelectionBar = Bar(data.width/40, data.height/5, data.width/5, data.height/6*5, 'SkyBlue1')
    data.buildRoadSelectionBar.draw(canvas, 'Road: 1L 1B')
    data.buildSettlementSelectionBar.draw(canvas)
    canvas.create_text(data.width/80*9, data.height/5*2, text='Settlement', font = 'Chalkduster 15')
    canvas.create_text(data.width/80*9, data.height/120*57, text='1L 1B 1G 1W', font='Chalkduster 15')
    ...
    if data.buildMode == 'road':
        canvas.create_text(data.width/40*7, data.height/15*2, text='BUILDING ROAD!!', font='Chalkduster 20', fill='blue')
    if data.buildMode == 'settlement':
        canvas.create_text(data.width/40*7, data.height/15*2, text='BUILDING SETTLEMENT!!', font='Chalkduster 20', fill='blue')
    if data.buildMode == 'city':
        canvas.create_text(data.width/40*7, data.height/15*2, text='BUILDING CITY!!', font='Chalkduster 20', fill='blue')
    if data.buildMode == 'moveMob':
        canvas.create_text(data.width/40*7, data.height/15*2, text='You have got Mob Card!', font='Chalkduster 20', fill='blue')
    if data.buildMode == 'feast':
        canvas.create_text(data.width/40*7, data.height/15*2, text='You have got Feast Card!', font='Chalkduster 20', fill='blue')
    if data.buildMode == 'extraVP':
        canvas.create_text(data.width/40*7, data.height/15*2, text='You have got an extra VP!', font='Chalkduster 20', fill='blue')
...
def drawTradeMenu(canvas, data):
    ...
    if data.tradeMode == 'bank':
        canvas.create_text(data.width/40*7, data.height/15*2, text='TRADE W/ BANK!!', font='Chalkduster 20', fill='blue')
        ...
        if data.giveOut == None:
            canvas.create_text(data.width/40*7+20, data.height/6, text='Select the Resource that you want to give', font='Chalkduster 13', fill='blue')
        if data.giveOut != None:
            canvas.create_text(data.width/40*7+20, data.height/6, text='Select the Resource that you want to take', font='Chalkduster 13', fill='blue')
    if data.tradeMode == 'port':
        canvas.create_text(data.width/40*7, data.height/15*2, text='TRADE W/ PORT!!', font='Chalkduster 20', fill='blue')
        data.woolBar.draw(canvas, 'Wool')
        data.brickBar.draw(canvas, 'Brick')
        data.grainBar.draw(canvas, 'Grain')
        data.oreBar.draw(canvas, 'Ore')
        ...
        if data.giveOut != None:
            canvas.create_text(data.width/40*7+20, data.height/6, text='Select the Resource that you want to take', font='Chalkduster 13', fill='blue')
    if data.tradeMode == 'player':
        canvas.create_text(data.width/40*7, data.height/15*2, text='TRADE W/ PLAYER!!', font='Chalkduster 20', fill='blue')
        if data.tradePID == None and data.tradedPID == None:
            ...
            canvas.create_text(data.width/40*7, data.height/6, text='Trade Accepted!!', font='Chalkduster 15', fill='blue')
        elif data.tradeRefused == True:
            canvas.create_text(data.width/40*7, data.height/6, text='Trade Refused!!', font='Chalkduster 15', fill='blue')
        elif data.takeIn != None and data.takeInQuantity != 0 and data.giveOut != None and data.giveOutQuantity != 0:
            canvas.create_text(data.width/40*7+20, data.height/6, text='%s wants to trade %d %s for %d %s from %s' % (data.tradePID, data.giveOutQuantity, data.giveOut, data.takeInQuantity, data.takeIn, data.tradedPID), font='Chalkduster 12', fill='blue')
...
def drawUseMenu(canvas, data):
    ...
    if data.feastGet != None:
        canvas.create_text(data.width/40*7, data.height/6, text='Congrats! You have got three %s' % (data.feastGet), font='Chalkduster 12', fill='blue')
    ...
    for i in range (len(data.resourceList)):
        if (not isinstance(data.resourceList[i], resourceClass.Seatile)) and (not isinstance(data.resourceList[i], resourceClass.TradingPort) and (not isinstance(data.resourceList[i], resourceClass.Desert))):
            if distance(x,y,data.resourceList[i].cx, data.resourceList[i].cy) < minDistance:
                minDistance = distance(x,y,data.resourceList[i].cx, data.resourceList[i].cy)
                newMobIndex = i
    return newMobIndex
    ...
    if (data.me.PID == data.playerList[data.turn%len(data.playerList)]) and not data.moveMob:
        ...
        if data.gameMoveMode == 'use':
            if data.me.moveMob > 0 and data.moveMobBar.inBar(event.x, event.y):
                ...
                data.me.feast -= 1 
                resource = random.choice(['wool','grain','ore','brick','lumber'])
        ...
        if data.gameMoveMode == 'build':
            if data.buildDevelopSelectionBar.inBar(event.x, event.y):
                if data.me.resource['grain'] < 1 or data.me.resource['wool'] < 1 or data.me.resource['ore'] < 1:
                    print('%s does not have enough resource to build development card' % data.me.PID)
                else:
                    n = random.random()
                    if 0 < n < 0.8:
            ...
            if data.buildMode == 'road':    
                if data.me.resource['lumber'] < 1 or data.me.resource['brick'] < 1:
                    print('%s does not have enough resource to build road' % data.me.PID)
                else:
                    ...
                    if len(data.roadPoint) == 2:
                        ...
                        if data.me.roadLocation != []:
                            ...
                            if isRoadBuiltByOthers(x1,y1,x2,y2,data.roadList):
                                ...
                                data.roadPoint = []
                    if len(data.roadPoint) == 2:
                        data.roadList.append(buildClass.Road(x1, y1, x2, y2, data.me.PID, data.me.color))
            ...
            if data.buildMode == 'settlement':
                if data.me.resource['lumber'] < 1 or data.me.resource['brick'] < 1 or data.me.resource['grain'] < 1 or data.me.resource['wool'] < 1:
                    print('%s does not have enough resource to build a settlement' % data.me.PID)
                else:
                    if data.me.findPoint(event, data) != None:
                        x = data.me.findPoint(event, data)[0]
                        y = data.me.findPoint(event, data)[1]
                        if isLegalSettlement(x,y,data):
                            data.settlementList.append(buildClass.Settlement(x,y,data.me.PID,data.me.color))
            ...
            if data.buildMode == 'city':
                if data.me.resource['grain'] < 2 or data.me.resource['ore'] < 3:
                    print('%s does not have enough resource to build a city' % data.me.PID)
                else:
                    if data.me.findPoint(event, data) != None:
                        x = data.me.findPoint(event, data)[0]
                        y = data.me.findPoint(event, data)[1]
                        if isLegalCity(x,y,data):
                            data.cityList.append(buildClass.City(x,y,data.me.PID,data.me.color))
        ...
        if data.gameMoveMode == 'trade':
            ...
            if data.closeTradeBar.inBar(event.x, event.y):
                ...
                data.tradeRefused = None
            if data.tradeMode == 'bank':
                data.woolBar = Bar(data.width/40, data.height/5, data.width/5, data.height/600*139)
                data.brickBar = Bar(data.width/40, data.height/600*139, data.width/5, data.height/600*158)
                data.grainBar = Bar(data.width/40, data.height/600*158, data.width/5, data.height/600*177)
                data.oreBar = Bar(data.width/40, data.height/600*177, data.width/5, data.height/600*196)
                data.lumberBar = Bar(data.width/40, data.height/600*196, data.width/5, data.height/120*43)
            ...
            if data.tradeMode == 'port':
                data.woolBar = Bar(data.width/40, data.height/120*43, data.width/5, data.height/100*39)
                data.brickBar = Bar(data.width/40, data.height/100*39, data.width/5, data.height/600*253)
                data.grainBar = Bar(data.width/40, data.height/600*253, data.width/5, data.height/75*34)
                data.oreBar = Bar(data.width/40, data.height/75*34, data.width/5, data.height/200*97)
                data.lumberBar = Bar(data.width/40, data.height/200*97, data.width/5, data.height/60*31)
                if data.giveOut == None and data.tradeStart:
                    if data.woolBar.inBar(event.x, event.y) and data.me.resource['wool'] >= 4:
                        data.giveOut = 'wool'
                    elif data.brickBar.inBar(event.x, event.y) and data.me.resource['brick'] >= 4:
                ...
                if data.tradeMode != None:
                    data.tradeStart = True
            if data.tradeMode == 'player':
                data.tradewP1Bar = Bar(data.width/40, data.height/60*31, data.width/5, data.height/160*89)
                data.tradewP2Bar = Bar(data.width/40, data.height/160*89, data.width/5, data.height/240*143)
                data.tradewP3Bar = Bar(data.width/40, data.height/240*143, data.width/5, data.height/96*61)
                data.tradewP4Bar = Bar(data.width/40, data.height/96*61, data.width/5, data.height/40*27)
                if data.tradePID == None and data.tradedPID == None:
                    if data.tradewP1Bar.inBar(event.x, event.y) and 'P1' in data.otherPlayers and data.tradeStart:
                        ...
                        data.tradedPID = 'P4'
                        data.tradeStart = False
                data.choose1Bar = Bar(data.width/40, data.height/60*31, data.width/5, data.height/600*329)
                data.choose2Bar = Bar(data.width/40, data.height/600*329, data.width/5, data.height/50*29)
                data.choose3Bar = Bar(data.width/40, data.height/50*29, data.width/5, data.height/600*367)
                data.choose4Bar = Bar(data.width/40, data.height/600*367, data.width/5, data.height/300*193)
                data.choose5Bar = Bar(data.width/40, data.height/300*193, data.width/5, data.height/40*27)
                if data.giveOutQuantity == 0 and data.tradeStart and data.tradePID != None:
                    if data.choose1Bar.inBar(event.x, event.y):
                        ...
                        data.tradeStart = False
                data.woolBar = Bar(data.width/40, data.height/60*31, data.width/5, data.height/600*329)
                data.brickBar = Bar(data.width/40, data.height/600*329, data.width/5, data.height/50*29)
                data.grainBar = Bar(data.width/40, data.height/50*29, data.width/5, data.height/600*367)
                data.oreBar = Bar(data.width/40, data.height/600*367, data.width/5, data.height/300*193)
                data.lumberBar = Bar(data.width/40, data.height/300*193, data.width/5, data.height/40*27)
...
def gamePlayRedrawAll(canvas, data):
    ...
    for city in data.cityList:
        city.draw(canvas)
    data.turnBar.draw(canvas, 'Turn %d: %s' % (data.turn, data.playerList[data.turn%len(data.playerList)]))
    ...
    canvas.create_text(data.width/40*39, data.height/12, text=str(data.dice), font='Chalkduster 20', fill='blue')
    ...
    if data.tradeNotification:
        canvas.create_text(data.width/40*7+20, data.height/15*2, text='Request from %s: Asking %d %s for %d %s' % (data.tradePID, data.takeInQuantity, data.takeIn, data.giveOutQuantity, data.giveOut), font='Chalkduster 13', fill='blue')
        canvas.create_text(data.width/40*7, data.height/6, text="Press 'Y' to accept, 'N' to reject" , font='Chalkduster 15', fill='blue')
    if data.moveMob:
        canvas.create_text(data.width/40*7, data.height/6, text="Please click to move the mob" , font='Chalkduster 15', fill='blue')
    if data.gameOver:
        canvas.create_text(data.width/2, data.height/2, text='Game Over. %s have conquered the island.' % (data.winnerID), font='Chalkduster 25', fill='blue')
